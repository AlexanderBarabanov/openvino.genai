---
name: update-docs
description: "Update OpenVINO GenAI documentation for API or feature changes. Use when: new pipelines, models, or use-cases are introduced; docs need to reflect new capabilities."
argument-hint: "Description of what changed (e.g. 'added SpeculativeDecodingPipeline' or 'changed GenerationConfig fields')"
---

# Update Docs

Updates documentation pages under `/docs/` after a code change. Documentation is built by the hub repository; this repo contains only content.

## When to Use

- New pipeline, model type, or use-case was introduced
- The supported-models table needs a new entry
- A new public API, config option, guide, or concept needs to be documented
...
Decide which sections need updating based on what changed:
...
- Cross-link to related pages using relative links (e.g., `[Supported Models](../supported-models/index.mdx)`).
- Images must be colocated next to docs in `img/` subdirectories and referenced with relative paths (e.g., `![Diagram](./img/diagram.svg)`).
- Do not use `@site/src/...` imports. Shared components (`Button`, `LanguageTabs`, `OptimumCLI`, etc.) are globally registered by the hub and available in MDX without imports.
...
- [ ] No `@site/src/` imports in any MDX file.
- [ ] Images use relative paths (`./img/...`), not absolute (`/img/...`).
